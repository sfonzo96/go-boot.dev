...
func getMessageWithRetriesForPlan(plan string) ([]string, error) {
	...
	// Slices are arrays like structures that has pointers to a underlaying array, it may not have all of its elements defined, unless arrays
	// Slices are defined over arrays, so the [:] operator allows to decide to which part of the array will the slice point at
	 if plan == planFree {
