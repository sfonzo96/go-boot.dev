...
Read/Write Mutexes (rwmutexes) allows us to block a resource from only writing, by using the RLock method on the rwmutexes (will indeed lock the resource to be written while other processes can still read from it), then unlocked by the RUnlock method. This behaviour is different from the Lock method that doesnt allow writing nor reading while the resource is locked.
*/
